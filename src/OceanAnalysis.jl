...
function as_ctd(salinity::Vector{Float64}, temperature::Vector{Float64}, pressure::Vector{Float64},
    ...
    oad(debug, "    given temperature (length: $(length(temperature)), max: $(maximum(filter(!isnan, temperature))))")
    ...
    oad(debug, "    created SA (length: $(length(SA)), max: $(maximum(filter(!isnan, SA))))")
...
function get_nc_value(item)
    ...
    if length(item) > 1
        item = convert(Vector{Float64}, item)
        item[item.>1.0e15] .= NaN
    else
        #println("is a scalar")
        item = convert(Float64, item)
        if item > 1.0e15
            item = NaN
        end
    end
