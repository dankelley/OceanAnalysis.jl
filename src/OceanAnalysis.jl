...
function as_Ctd(salinity::Vector{Float64}, temperature::Vector{Float64}, pressure::Vector{Float64},
    ...
    oad(debug, "    salinity length: ", @eval length(salinity))
    oad(debug, "    temperature length: ", @eval length(temperature))
    oad(debug, "    pressure length: ", @eval length(pressure))
    oad(debug, "    longitude length: ", @eval length(longitude))
    oad(debug, "    latitude length: ", @eval length(latitude))
    ...
    oad(debug, "_CT length: ", @eval length(CT))
    #oad(debug, "in Ctd() at line 105")
    spiciness0 = gsw_spiciness0.(SA, CT)
    oad(debug, "_spiciness0 length: ", length(spiciness0))
    sigma0 = gsw_sigma0.(SA, CT)
    oad(debug, "_sigma0 length: ", length(sigma0))
    oad(debug, "_assembling metadata")
    metadata = Dict{String,Any}()
    metadata["longitude"] = longitude
    metadata["latitude"] = latitude
    oad(debug, "_assembling data")
