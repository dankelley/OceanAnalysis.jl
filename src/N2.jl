...
- `s` either a Float64 value or a symbol. In the first case, it is the value of  `s` supplied to `Dierckx::Spline1D()`, which is used to smooth the density curve as a function of pressure.  According to the documentation for the Fortran code behind this function (see [https://www.netlib.org/dierckx/curfit.f](https://www.netlib.org/dierckx/curfit.f)), a reasonable starting point for exploring the dependence of the spline curve on `s` is in the range from ``\\delta^2 (N-\\sqrt{2N})`` to ``\\delta^2 (N+\\sqrt{2N})``, where ``N`` is the number of data points and ``\\delta`` is a measure of the density "wiggles" (anomalies or high-wavenumber signals) to be smoothed across in the spline. The midpoint of this range is used if `s=:auto` (the default) is specified; using `s=:smooth` and `s=:rough` multiplies this value by sqrt(2) and 1/sqrt(2), respectively. The user is also free to specify the value of `s` numerically, if the three preset choices are not judged to be suitable.
