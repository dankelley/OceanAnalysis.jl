...
- `s` either a Float64 value or a symbol. In the first case, it is the value of  `s` supplied to `Dierckx::Spline1D()`, which is used to smooth the density curve as a function of pressure.  According to the documentation for the Fortran code behind this function (see [https://www.netlib.org/dierckx/curfit.f](https://www.netlib.org/dierckx/curfit.f)), a reasonable starting point for exploring the dependence of the spline curve on `s` is in the range from ``\\delta^2 (N-\\sqrt{2N})`` to ``\\delta^2 (N+(2N)^{1/2})``, where ``N`` is the number of data points and ``\\delta`` is a measure of the density "wiggles" across which to smooth the spline. This is the value used if `s=:auto` (the default) is specified; using `s=:smooth` and `s=:rough` multiplies this value by sqrt(2) and 1/sqrt(2), respectively. Note that specifying `debug=1` will make the function print out the numeric value of `s` used, if one of these three symbols has been supplied.
