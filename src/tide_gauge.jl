...
This function depends on information provided at References 1 and 2, and so if
it stops working, a good plan would be to consult these references to
see if the data format (or URL/API structure) had changed since autum
2026, when the function was written.
...
2. General information about the data provided by CHS is given at
...
function get_tide_gauge_index(search=:all; debug::Integer=0)
    ...
    oad(debug, "    Assembling results in a DataFrame")
    if search == :all
...
- `times` an indication of the start and end times for which data are sought.
  ...
  If `resolution=1` then `times[1]` is set to 1 week ago.
...
```julia
# Show 3 week of 3-minute-sampled sealevel in Bedford Basin, Nova Scotia
using OceanAnalysis, CSV, DataFrames
using GLMakie # or CairoMakie
name, csv = get_tide_gauge_file("Bedford")
...
function get_tide_gauge_file(search; times=:default, variable=:wlo, resolution=3, debug::Integer=0)
    ...
    if times == :default
        ...
        elseif resolution == 3
            time_start = time_end - Week(2)
        else
            time_start = time_end - Month(1)
        end
    ...
    i = get_tide_gauge_index(search, debug=increment_debug(debug))
    nr = nrow(i)
    ...
    oad(debug, "    Downloaded $n lines of data")
    ...
    rval = (station_name=i[1, :name], file=filename)
...
- `search` a String specifying the name of a tide gauge. If more
  than one gauge matches the string, an error is reported, in
  which the partial matches are listed.
