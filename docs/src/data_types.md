...
- [`Ctd`](@ref) holds hydrographic data, from plain CTD instruments or Argo
  profilers.  In each case, the `metadata` item holds information about the
  data (such as the position and time of sampling), and the `data` item is a
  DataFrame with columns named `salinity`, `temperature` and `pressure`, along
  (in some cases with other columns in the data file). The following functions
  return [`Ctd`](@ref) values: [`as_ctd`](@ref), [`read_ctd_cnv`](@ref), and
  [`read_argo`](@ref).  Objects of type [`Ctd`](@ref) may be plotted with
